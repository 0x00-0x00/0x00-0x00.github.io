...
The function used to decrypt the data, using the current-user DPAPI key is [this one](https://docs.microsoft.com/en-us/dotnet/api/system.security.cryptography.protecteddata.unprotect?view=netframework-4.7.2).
...
All code samples I found in the internet used PsExec to get a system shell. I dislike this method, and prefered to create a smooth experience by not relying on any external tool like tools from SysInternals. So I chose to use Token Impersonation from my "__How to get system - Part 2__" as it was working and only relies on PowerShell.
This resulted in the following PowerShell script:
...
![Screenshot](/assets/enterprise-01.jpg)
